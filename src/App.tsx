  ...
  const executeCommand = (command: string) => {
    ...
    if (options.includes(command)) {
      ...
      if (command === "help") {
        ...
        setHistory((history) => [...history, { command: command, output: "" }]);
      } else {
        setHistory((history) => [
          ...history,
          { command: command, output: "command not found" },
        ]);
      }
    }
  };
  ...
  const Prompt = () => {
    return (
      <span className="mr-1">
        ...
        :<span className="text-blue-700">{path}</span> {symbol}{" "}
      </span>
    );
  };
  return (
    <div className="font-bold text-xl p-2">
      {" "}
      {
        /* History */ history.map((history) => (
          <div className=" mb-2">
            {" "}
            <Prompt /> <span>{history.command}</span> <br />{" "}
